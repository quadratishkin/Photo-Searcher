...
# ========== ТЕСТ ДЛЯ ЛОКАЛЬНОГО ФАЙЛА nikita.jpg ==========
if __name__ == "__main__":
    import os
    
    print("=" * 60)
    print("🧪 ТЕСТ PhotoHandler ДЛЯ ЛОКАЛЬНОГО ФАЙЛА nikita.jpg")
    print("=" * 60)
    
    # Путь к файлу в текущей директории
    current_dir = os.path.dirname(os.path.abspath(__file__))
    image_path = os.path.join(current_dir, "nikita.jpg")
    
    # Проверяем существует ли файл
    if not os.path.exists(image_path):
        print(f"\n❌ Файл не найден: {image_path}")
        print("\n💡 Положите файл nikita.jpg в папку:")
        print(f"   {current_dir}")
        print("=" * 60)
        sys.exit(1)
    ...
    try:
        ...
        print(f"\n📥 1. Загрузка изображения: {image_path}")
        img = Image.open(image_path)
        print(f"    Размер: {img.size}")
        print(f"    Формат: {img.format}")
        
        # Шаг 2: Инициализация обработчика
        print("\n🤖 2. Инициализация PhotoHandler...")
        handler = PhotoHandler()
        
        # Шаг 3: Генерация описания
        print("\n🔍 3. Генерация описания...")
        description = handler.get_description(img)
        ...
        # Дополнительно: покажем оригинал на английском
        print(f"\n📝 Оригинал (EN): {handler.describe_combined(img)}")
