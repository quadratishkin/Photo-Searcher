...
def main():
    ...
    print("Пример 1: Обнаружение лиц на одном изображении")
    image_path = r"C:\Users\vadim\PycharmProjects\Photo-Searcher\backend\searcher_app\utils\orig.jpg"

    try:
        # Детекция лиц и получение эмбеддингов
        boxes, embeddings = detector.detectFaces(image_path)

        if boxes is not None:
            print(f"Найдено лиц: {len(boxes)}")
            print(f"Координаты лиц: {boxes}")
            if embeddings is not None and len(embeddings) > 0:
                print(f"Эмбеддинги (формат): {embeddings.shape}")
        else:
    ...
    # Пример 2: Получение количества лиц
    print("\nПример 2: Подсчет лиц на изображении")
    try:
        face_count = detector.getFaceCount(image_path)
        print(f"Количество лиц: {face_count}")
    except Exception as e:
        print(f"Ошибка: {e}")
